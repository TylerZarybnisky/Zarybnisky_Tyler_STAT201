...
while (sum < 30){
  new_num <- as.integer(runif(1, 0, 10))+1
  sum <- sum + new_num
  print(new_num)
  count <- count + 1
}

print(paste0("You generated ", count, " numbers that sum to ", sum, "."))
...
BMI_calc <- function(w, h){
  ...
  return_vector <- c(BMI, category)
  
  return (return_vector)
}

cat("Your BMI is", BMI_calc(170, 71)[2], "with a value of", as.integer(10*(as.numeric(BMI_calc(170, 71)[1])))/10)
...
mat_of_cols <- cbind(vec1, vec2, vec3)
...
max_col <- apply(mat_of_cols, 2, max)
...
list_sum <- lapply(list1, sum)
vec_storage <- c()
for (i in 1:length(list_sum)){
  list_sum[[i]] <- list_sum[[i]] %% 2 == 0
  vec_storage <- c(vec_storage, list_sum[[i]])
}
...
for(i in inx_lst){
fruit_new <- c(fruit_new, fruit_info[i, "fruit"])
color_new <- c(color_new, fruit_info[i, "color"])
weight_new <- c(weight_new, fruit_info[i, "weight"])
}

fruits_red <- data.frame(
  "fruit" = fruit_new[1:6],
  "color" = color_new[1:6],
  "weight" = weight_new[1:6]
)
...
max <- fruit_info[1, "weight"]
indc <- 1

for (i in 2:nrow(fruit_info)){
  if (fruit_info[i, "weight"] > max){
    max <- fruit_info[i, "weight"]
    indc <- i
  }
}

print(fruit_info[indc, "fruit"])
