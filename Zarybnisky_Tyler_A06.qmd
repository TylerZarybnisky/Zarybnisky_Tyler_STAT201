...
max_diff <- bears[[1]] - opponent[[1]]
week_of <- 1

for (i in 2:length(bears)){
  if (bears[[i]]-opponent[[i]] > max_diff){
    max_diff <- bears[[i]]-opponent[[i]]
    week_of <- i
  }
}

cat(paste0("The largest win spread is ", max_diff, ", when the Bears scored ", bears[[week_of]], " points and the Opponent scored ", opponent[[week_of]], " points."))
...
mat_add <- matrix(c(37, 9, 17, 17), nrow = 2)
game_score <- rbind(game_score, mat_add)
...
cat("The Bears lost", num_losses, "games before they won their first one.")
...
number_of_factors <- function(int1){
  num_factors <- 0
  for (i in 1:int1){
    if (int1 %% i == 0){
...
area_function <- function(l = 48, w = 48){
  output <- c(l*w, l*w/144)
  return(output)
}

v_output <- area_function(102, 60)

print(paste0("The area in square inches is ", v_output[[1]], "."))
print(paste0("The area in square feet is ", v_output[[2]], "."))
...
most_lets <- function(word_list){
  max_letters <- length(word_list[[1]])
  longest_words <- c()
  for (i in word_list){
    if (nchar(i) > max_letters){
      max_letters <- nchar(i)
    }
  }
  for (i in word_list){
    if (nchar(i) == max_letters){
      longest_words <- append(longest_words, i)
    }
  }
  return(longest_words)
}

print(most_lets(fruits))
...
bool_result <- vec_check(value, check)
